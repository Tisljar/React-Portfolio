...
const Navbar = ({ isTopOfPage, selectedPage, setSelectedPage }) => {
    ...
    const navbarBackground = isTopOfPage ? 'bg-dark-bg' : 'bg-dark-bg';
    return (
        <nav className={`${navbarBackground} z-40 w-full fixed top-0 py-6`}>
            <div className="flex items-center justify-between mx-auto w-5/6">
                <h4 className="font-playfair text-3xl font-bold">DT</h4>
                {isAboveSmallScreens ? (
                ...
                {!isAboveSmallScreens && isMenuToggled && (
                    <div className="fixed right-0 bottom-0 h-full bg-light-grey w-full">
                        <div className="flex justify-end p-12">
                            <button onClick={() => setIsMenuToggled(!isMenuToggled)}>
                                <img alt="menu" src="../assets/close-icon.svg"></img>
                            </button>
                        </div>
